...
void SpatialExpanderAudioProcessor::parameterChanged (const juce::String& parameterID, float /*newValue*/)
{
    ...
    else if (parameterID == "crosstalk" || parameterID == "stretch" || parameterID == "rearBias")
    {
        if (calState.load() != CalState::Normal)
            return;
                                                ...
                                                int fftSize, int numSpecOut)
{
    ...
    // --- Rear Bias: linear complex blend (global loudness handled by calibration) ---
    float rearBias = apvts.getRawParameterValue ("rearBias")->load();

    if (rearBias > 0.0f)
    {
        // DC
        fftRearL[0] = fftRearL[0] * (1.0f - rearBias) + cascadeLresSave[0] * rearBias;
        fftRearR[0] = fftRearR[0] * (1.0f - rearBias) + cascadeRresSave[0] * rearBias;

        // Nyquist
        if (fftSize > 2)
        {
            fftRearL[1] = fftRearL[1] * (1.0f - rearBias) + cascadeLresSave[1] * rearBias;
            fftRearR[1] = fftRearR[1] * (1.0f - rearBias) + cascadeRresSave[1] * rearBias;
        }
        ...
        for (int k = 1; k < fftSize / 2; ++k)
        {
            ...
            fftRearL[idx]     = fftRearL[idx]     * (1.0f - rearBias) + cascadeLresSave[idx]     * rearBias;
            fftRearL[idx + 1] = fftRearL[idx + 1] * (1.0f - rearBias) + cascadeLresSave[idx + 1] * rearBias;
            fftRearR[idx]     = fftRearR[idx]     * (1.0f - rearBias) + cascadeRresSave[idx]     * rearBias;
            fftRearR[idx + 1] = fftRearR[idx + 1] * (1.0f - rearBias) + cascadeRresSave[idx + 1] * rearBias;
        }
    }
